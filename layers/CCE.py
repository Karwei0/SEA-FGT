...
def test_cce_revised():
    ...
    cce = CCE(
        ...
        T=seq_length,
        bin_size=2,
        use_laplacian=False  # 使用相干矩阵A而不是拉普拉斯特征向量
    )
    ...
    # 前向传播
    x_transformed, coherence_matrix = cce(x)

    print(f"输入形状: {x.shape}")
    print(f"输出序列形状: {x_transformed.shape}")
    print(f"相干矩阵形状: {coherence_matrix.shape}")

    # 验证输出形状
    assert x_transformed.shape == (batch_size, num_channels, seq_length)
    print('coherence_matrix shape: ', coherence_matrix.shape)
    assert coherence_matrix.shape == (batch_size, num_channels, num_channels)

    print("测试通过！修订后的CCE模块工作正常。")
